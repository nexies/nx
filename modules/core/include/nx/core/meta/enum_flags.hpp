...
#ifndef NX_ENUM_FLAGS_HPP
#define NX_ENUM_FLAGS_HPP
...
namespace nx::detail
{
    ...
    public:
        ...
        // // Enum <op> Enum -> Flags (чтобы писать E1 | E2)
        friend constexpr make_flags_from_enum operator|(const Enum a, const Enum b) noexcept
        {
            return make_flags_from_enum(a) | b;
        }
        friend constexpr make_flags_from_enum operator&(const Enum a, const Enum b) noexcept
        {
            return make_flags_from_enum(a) & b;
...
#define _nx_make_enum_flags(Enum, Flags) \
    using Flags = ::nx::detail::make_flags_from_enum<Enum>;


    // constexpr Flags operator|(const Enum a, const Enum b) noexcept { \
    //     return Flags { static_cast<Flags::underlying_type>(a) | static_cast<Flags::underlying_type>(b) }; \
    // } \
    // constexpr Flags operator|(const Enum a, const Enum b) noexcept { \
    //     return Flags { static_cast<Flags::underlying_type>(a) | static_cast<Flags::underlying_type>(b) }; \
    // } \
...
#define NX_FLAGS(Enum, Flags) \
    _nx_make_enum_flags(Enum, Flags)
