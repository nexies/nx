 ...
 *          - Знает, в каком nx::Loop он находится (nx::LoopPool? nx::LoopManager? nx::AppManager? )
 ...
 *          - наследник nx::Object ! Знает, в каком Loop был создан - соответственно строится дерево зависимостей nx::Thread
 *
 *      nx::Loop   - Объект, представляющий собой цикл ивентов, занимающий собой выделенный ему поток (может быть локальный)
 *          - Имеет очередь событий для обработки
