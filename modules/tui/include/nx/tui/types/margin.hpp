...
struct margin {
    ...
    explicit constexpr margin(int all) noexcept
        : top(all), right(all), bottom(all), left(all) {}

    // Axis-based — vertical (top/bottom) and horizontal (left/right).
    constexpr margin(int vertical, int horizontal) noexcept
        : top(vertical), right(horizontal), bottom(vertical), left(horizontal) {}

    // Per-side (CSS order: top, right, bottom, left).
    constexpr margin(int t, int r, int b, int l) noexcept
        : top(t), right(r), bottom(b), left(l) {}

    // ── Queries ───────────────────────────────────────────────────────────────

    [[nodiscard]] constexpr int horizontal() const noexcept { return left + right;  }
    [[nodiscard]] constexpr int vertical()   const noexcept { return top  + bottom; }

    [[nodiscard]] constexpr bool is_zero() const noexcept
    { return top == 0 && right == 0 && bottom == 0 && left == 0; }

    [[nodiscard]] constexpr bool operator==(const margin & o) const noexcept
