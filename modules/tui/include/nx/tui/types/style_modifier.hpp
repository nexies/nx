...
namespace nx::tui {
    ...
    namespace detail {
        inline style_option
        make_modifier(std::shared_ptr<style_modifier> m) {
            style_option s;
    ...
    NX_NODISCARD inline style_option
    h_gradient(target t, color from, color to) { return gradient(axis::horizontal, t, from, to); }

    NX_NODISCARD inline style_option
    v_gradient(target t, color from, color to) { return gradient(axis::vertical, t, from, to); }

    NX_NODISCARD inline style_option
    h_gradient_fg(color from, color to) { return gradient(axis::horizontal, target::fg, from, to); }

    NX_NODISCARD inline style_option
    h_gradient_bg(color from, color to) { return gradient(axis::horizontal, target::bg, from, to); }

    NX_NODISCARD inline style_option
    v_gradient_fg(color from, color to) { return gradient(axis::vertical, target::fg, from, to); }

    NX_NODISCARD inline style_option
    v_gradient_bg(color from, color to) { return gradient(axis::vertical, target::bg, from, to); }
    ...
    NX_NODISCARD inline style_option
    brightness(target t, float value) {
        return detail::make_modifier(
            std::make_shared<brightness_modifier>(t, value));
    }

    NX_NODISCARD inline style_option
    h_brightness(target t, float from, float to) { return brightness(axis::horizontal, t, from, to); }

    NX_NODISCARD inline style_option
    v_brightness(target t, float from, float to) { return brightness(axis::vertical, t, from, to); }

    NX_NODISCARD inline style_option
    h_brightness_fg(float from, float to) { return brightness(axis::horizontal, target::fg, from, to); }

    NX_NODISCARD inline style_option
    h_brightness_bg(float from, float to) { return brightness(axis::horizontal, target::bg, from, to); }

    NX_NODISCARD inline style_option
    v_brightness_fg(float from, float to) { return brightness(axis::vertical, target::fg, from, to); }

    NX_NODISCARD inline style_option
    v_brightness_bg(float from, float to) { return brightness(axis::vertical, target::bg, from, to); }

    // Single-value shortcuts (static, no gradient).
    NX_NODISCARD inline style_option
    brightness_fg(float v) { return brightness(target::fg, v); }

    NX_NODISCARD inline style_option
    brightness_bg(float v) { return brightness(target::bg, v); }

    NX_NODISCARD inline style_option
    brightness_all(float v) { return brightness(target::both, v); }
} // namespace nx::tui
