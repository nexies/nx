...
public:
    ...
    [[nodiscard]] const style_option & get_style(theme_role r) const noexcept;

    // Returns the full style_option for any role (built-in or custom).
    // Returns an empty style_option for unknown custom roles.
    [[nodiscard]] style_option get_style(uint32_t r) const noexcept;

    // Convenience: foreground color of the role's style.
    [[nodiscard]] color get_color(theme_role r) const noexcept;

    // Convenience: background color of the role's style.
    [[nodiscard]] color get_bg(theme_role r) const noexcept;

    // ── Mutation ──────────────────────────────────────────────────────────────

    theme & set(theme_role r, style_option s) noexcept;

    // Generic overload for user-defined roles (any uint32_t-convertible enum).
    template <typename R>
    theme & set(R r, style_option s) noexcept
    {
        return set(static_cast<uint32_t>(r), std::move(s));
    }

    theme & set(uint32_t r, style_option s) noexcept;

    // Convenience setters (only touch the fg / bg of an existing entry).
    theme & set_color(theme_role r, color fg) noexcept;
    theme & set_bg   (theme_role r, color bg) noexcept;
    theme & set_colors(theme_role r, color fg, color bg) noexcept;

    // Generic getter for user-defined roles.
    template <typename R>
    [[nodiscard]] style_option get_style(R r) const noexcept
    {
        return get_style(static_cast<uint32_t>(r));
    }

    template <typename R>
    [[nodiscard]] color get_color(R r) const noexcept
    {
        return get_style(r).foreground.value_or(color::default_color);
    }

    template <typename R>
    [[nodiscard]] color get_bg(R r) const noexcept
    {
